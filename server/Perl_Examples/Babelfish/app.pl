...
my $hangup_app = sub {
	...
	my $response = $sw->POST( "calls", id => $data->{uuid}, command => "calling.end", params => { reason => "hangup" } );
...
my $call_app = sub {
	...
	my $response = $sw->POST( "calls", command => "dial", params => { swml => $swml->render, to => "$data->{callee}", from => "$ENV{FROM_NUMBER}" } );
...
my $events_app = sub {
	...
	my $template = HTML::Template::Expr->new( filename => "/app/template/events.tmpl", die_on_bad_params => 0 );
...
#			    Domain 	=> $ENV{DOMAIN_NAME},
			...
			my $response = $sw->POST( "calls", id => $uuid, command => "calling.live_translate", params => { action => { inject => { message => "$message", direction => "remote-caller"} } } );
			print STDERR Dumper( $sw );
			print STDERR Dumper( $response );
